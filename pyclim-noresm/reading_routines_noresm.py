...
def make_filelist_cmor(self, var,  activity_id='CMIP',  realm = 'Amon', grid = 'gn', path_to_data = '/projects/NS9034K/CMIP6/'):
        ...
        self.realm = realm
        self.grid = grid
        # Need some sort of selection function since the ocean grid names are kind of variable, can also have multiple grids for one given variable:
...
def read_noresm_cmor(model, varlist, realm = 'Amon', grid = 'gn', dim='time', transform_func=None):
    ...
    Please NOTE: It is not a general CMIP6 reading routine as it contains NorESM specific bug fixes
    ...
    for var in varlist:
        ...
        with xr.open_mfdataset(model.filenames, concat_dim=dim, combine="nested",
            ...
            if 'height' in ds.variables:
                # Datasets for variables @ a given height 2m temperature, 10m surface temperature 
                ...
                # The height is not really used for anything, so safe to drop it
                ds = ds.drop('height')
...
class Modelinfo:
    ...
    def __init__(self, name = 'NorESM2-LM', institute = 'NCC', activity_id = 'CMIP', expid='piControl', 
        ...
        '''
        The attributes need to be general and not file specific
        ...
        branchtime_year : int, when simulation was branched off from parent. 
                          ...
                          e.g. piControl_data_array(year = slice(branchtime_year,None))
